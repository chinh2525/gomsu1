...
function woo_breadcrumbs( $args = array() ) {
	...
	$defaults = array(
		...
		'after' => false,
		'front_page' => true,
		'show_home' => esc_html__( 'Home', 'flip' ),
		'echo' => true,
	...
	if ( !is_front_page() && $show_home )
		$trail[] = '<a href="' .  esc_url( home_url() ) . '?lang='.ICL_LANGUAGE_CODE.'" title="' . esc_attr( get_bloginfo( 'name' ) ) . '" rel="home" class="trail-begin">' . esc_html( $show_home ) . '</a>';
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( esc_html__( 'g:i a', 'flip' ) );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( esc_html__( 'Minute %1$s', 'flip' ), get_the_time( esc_html__( 'i', 'flip' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( esc_html__( 'g a', 'flip' ) );
		}
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'flip' ) ) . '">' . get_the_time( esc_html__( 'Y', 'flip' ) ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'flip' ) ) . '">' . get_the_time( esc_html__( 'F', 'flip' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( esc_html__( 'j', 'flip' ) );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'flip' ) ) . '">' . get_the_time( esc_html__( 'Y', 'flip' ) ) . '</a>';
				$trail['trail_end'] = sprintf( esc_html__( 'Week %1$s', 'flip' ), get_the_time( esc_attr__( 'W', 'flip' ) ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'flip' ) ) . '">' . get_the_time( esc_html__( 'Y', 'flip' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( esc_html__( 'F', 'flip' ) );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( esc_html__( 'Y', 'flip' ) );
			}
		}
	}
	...
	elseif ( is_search() )
		$trail['trail_end'] = sprintf( esc_html__( 'Search results for &quot;%1$s&quot;', 'flip' ), esc_attr( get_search_query() ) );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = esc_html__( '404 Not Found', 'flip' );
...
function wf_maybe_add_shop_page_link ( $trail ) {
	...
	return $trail;
} // End wf_maybe_add_shop_page_link()
}
...
function woo_get_term_parents( $id, $taxonomy, $link = false, $separator = '/', $nicename = false, $visited = array() ) {
	...
	if ( $link ) {
		$chain .= '<a href="' . get_term_link( $parent, $taxonomy ) . '" title="' . esc_attr( sprintf( esc_html__( 'View %s', 'flip' ), $parent->name ) ) . '">' . esc_html( $parent->name ) . '</a>' . $separator;
	} else {
